...
The term $R_{t+1}+\gamma \hat V(S_{t+1}) - \hat V(S_t)$ is also called the \emph{advantage} of performing action $A_t$ in state $S_t$, since it compares the estimated value of the state where performing $A_t$ would bring us to the value of the current state $S_t$:
$$
\mathcal{A}(S_t, A_t) = R_{t+1}+\gamma \hat V(S_{t+1}) - \hat V(S_t)
$$
